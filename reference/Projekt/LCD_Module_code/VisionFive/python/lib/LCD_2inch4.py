...
class LCD_2inch4():
    def __init__(self):
        self.np=lcdconfig.np
        self.RST_PIN = lcdconfig.RST_PIN
        self.DC_PIN = lcdconfig.DC_PIN
        self.DC_PIN = lcdconfig.DC_PIN
        self.BL_PIN = lcdconfig.BL_PIN
        self.width = width
        self.height = height

    def command(self, cmd):
        lcdconfig.digital_write(self.DC_PIN, 0)
        lcdconfig.spi_writebyte([cmd])

    def data(self, val):
        lcdconfig.digital_write(self.DC_PIN, 1)
        lcdconfig.spi_writebyte([val])
    def reset(self):
        """Reset the display"""
        lcdconfig.GPIO.output(self.RST_PIN,1)
        time.sleep(0.01)
        lcdconfig.GPIO.output(self.RST_PIN,0)
        time.sleep(0.01)
        lcdconfig.GPIO.output(self.RST_PIN,1)
        time.sleep(0.01)
    ...
    def ShowImage(self,Image,Xstart=0,Ystart=0):
        ...
        if imwidth == self.height and imheight ==  self.width:
            ...
            lcdconfig.digital_write(self.DC_PIN,self.GPIO.HIGH)
            for i in range(0,len(pix),4096):
                lcdconfig.spi_writebyte(pix[i:i+4096])
    ...
    def clear(self):
        ...
        for i in range(0,len(_buffer),4096):
            lcdconfig.spi_writebyte(_buffer[i:i+4096])	
        
    def module_exit(self):
        lcdconfig.module_exit()
        
    def bl_DutyCycle(self, duty):
       lcdconfig.bl_DutyCycle(duty)
        
    def bl_Frequency(self, freq):
        lcdconfig.bl_Frequency(freq)